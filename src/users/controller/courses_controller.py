...
def insert_courses():
    ...
    existing_course = repository.find_by_name (courses_data["name"])
    if existing_course:
        return jsonify({"message": "El curso ya existe"}), 409  # Conflict error code

    # Create a new course and add it to the repository
    new_course = repository.add(courses_data["name"], courses_data["descripcion"])
